...
const QuadraticView: React.FC = () => {
  const [coeffs, setCoeffs] = useState({ a: 1, b: 0, c: -5 }); // x^2 - 5 = 0 (Kökler: ±√5)
  ...
  return (
    <div className="space-y-4 md:space-y-6 animate-in slide-in-from-right duration-500">
      <div className="bg-white dark:bg-slate-900 p-5 md:p-8 rounded-[2.5rem] shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        <div className="bg-slate-900 dark:bg-black text-white rounded-[3rem] p-6 md:p-12 flex flex-col items-center justify-center text-center shadow-2xl relative overflow-hidden border border-white/5">
          <div className="absolute top-0 right-0 p-6 opacity-5 pointer-events-none">
            <i className="fa-solid fa-square-root-variable text-[120px]"></i>
          </div>
          
          <div className="text-indigo-400 dark:text-indigo-300 font-mono mb-2 text-2xl md:text-5xl tracking-tight font-black transition-all duration-300">
            {formatEquation()}
          </div>
          
          <div className="w-24 h-1.5 bg-gradient-to-r from-transparent via-indigo-500 to-transparent rounded-full my-6 md:my-8"></div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 w-full mb-8">
            <div className="p-5 md:p-7 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm group hover:bg-white/10 transition-colors">
              <span className="text-slate-400 text-[10px] font-black uppercase block mb-1 tracking-[0.2em]">Diskriminant (Δ)</span>
              <span className="text-3xl md:text-5xl font-black font-mono text-white group-hover:text-indigo-300 transition-colors">{result.discriminant}</span>
              <p className="text-[9px] text-slate-500 mt-2 uppercase tracking-widest font-bold">Δ = b² - 4ac</p>
            </div>
            <div className="p-5 md:p-7 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-sm flex flex-col justify-center">
              <span className="text-slate-400 text-[10px] font-black uppercase block mb-1 tracking-[0.2em]">Çözüm Özeti</span>
              <span className="text-base md:text-xl font-bold text-indigo-200 leading-tight italic">{result.message}</span>
            </div>
          </div>
          ...
          {result.symbolicRoots.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 w-full">
              {result.symbolicRoots.map((sRoot, i) => (
                <div key={i} className="bg-gradient-to-br from-indigo-600 to-indigo-900 p-6 md:p-8 rounded-[2.5rem] flex flex-col items-center shadow-2xl transform hover:scale-[1.02] transition-all border border-white/20 relative overflow-hidden group">
                  <div className="absolute -right-4 -top-4 opacity-10 group-hover:rotate-12 transition-transform">
                    <i className="fa-solid fa-hashtag text-6xl"></i>
                  </div>
                  <span className="text-indigo-200 text-[10px] font-black uppercase tracking-[0.3em] mb-3">Denklem Kökü x{i+1}</span>
                  <span className="text-3xl md:text-4xl font-black font-serif text-white drop-shadow-md">
                    {sRoot}
                  </span>
                  <div className="mt-4 pt-4 border-t border-white/10 w-full">
                    <span className="text-[11px] text-indigo-300/80 font-mono font-bold">
